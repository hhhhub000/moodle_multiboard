    ...
    var Board = {
        ...
        registerEvents: function () {
            ...
            $(document).on('click', '.edit-note', function () {
                ...
                var content = note.find('.note-content').text().trim();
                ...
                if (response.notes.length > 0 || response.comments.length > 0) {
                    ...
                    response.notes.forEach(function (note) {
                        if ($(`.multiboard-note[data-note-id="${note.id}"]`).length === 0) {
                            ...
                            var currentColumn = $(`.multiboard-note[data-note-id="${note.id}"]`).closest('.multiboard-column').data('column-id');
                            if (currentColumn != note.columnid) {
                                var el = $(`.multiboard-note[data-note-id="${note.id}"]`).detach();
